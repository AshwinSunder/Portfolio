...
function plot_precicemesh(precicemesh_data, plot_data, leg_list)
   ...
   % Initialize plot max starting and ending time steps
    ...
    for i = 1:length(plot_data.fields)
        ...
        for j = 1:length(precicemesh_data) 
            ...
            if (precicemesh_data{j}.iter-1)*precicemesh_data{j}.dt < t_end
                t{j} = reshape(t_start:precicemesh_data{j}.dt:(precicemesh_data{j}.iter-1)*precicemesh_data{j}.dt,[],1);
            else
                t{j} = reshape(t_start:precicemesh_data{j}.dt:t_end,[],1);
            end
            if t_max <= t{j}(end)
                t_max = t{j}(end)+precicemesh_data{j}.dt;
            end
        ...
        for j = 1:plot_data.nnodes
            for k = 1:length(precicemesh_data)
                ...
                for l = t{k}(1)/precicemesh_data{k}.dt+1:t{k}(end)/precicemesh_data{k}.dt+1
                                            
                    % Moment due to forces on all nodes need to be calculated
                    if matches(plot_data.fields{i}, 'Moment')     
                        posx(k) = precicemesh_data{k}.precicemesh{j}(l).('PositionX');
                        posy(k) = precicemesh_data{k}.precicemesh{j}(l).('PositionY');
                        posz(k) = precicemesh_data{k}.precicemesh{j}(l).('PositionZ');
    
                        momx(k) = (precicemesh_data{k}.precicemesh{j}(l).('ForceZ').*posy(k)) - (precicemesh_data{k}.precicemesh{j}(l).('ForceY').*posz(k));          
                        momy(k) = (precicemesh_data{k}.precicemesh{j}(l).('ForceX').*posz(k)) - (precicemesh_data{k}.precicemesh{j}(l).('ForceZ').*posx(k));            
                        momz(k) = (precicemesh_data{k}.precicemesh{j}(l).('ForceY').*posx(k)) - (precicemesh_data{k}.precicemesh{j}(l).('ForceX').*posy(k)); 
                    end

                    % In each iteration, extract x,y and z values from each struct
                    val{k}(1,j,l) = precicemesh_data{k}.precicemesh{j}(l).(plot_data.fields{i} + "X");
                    val{k}(2,j,l) = precicemesh_data{k}.precicemesh{j}(l).(plot_data.fields{i} + "Y");
                    val{k}(3,j,l) = precicemesh_data{k}.precicemesh{j}(l).(plot_data.fields{i} + "Z");

                    % Sum up values from each node
                    sum_val{k}(1,l) = sum_val{k}(1,l) + val{k}(1,j,l) + momx(k);
                    sum_val{k}(2,l) = sum_val{k}(2,l) + val{k}(2,j,l) + momy(k);
                    sum_val{k}(3,l) = sum_val{k}(3,l) + val{k}(3,j,l) + momz(k); 
        ...
        for j = 1:length(precicemesh_data)
            for k = 1:(length(coord)+plot_data.mag)*(length(nodes)+plot_data.sum)
                figure(k);
                plot(t{j}, plot_val{k,j}(t_start/precicemesh_data{j}.dt+1:t_start/precicemesh_data{j}.dt+length(t{j})));
                title(tit{k});
                xlabel('Time');
