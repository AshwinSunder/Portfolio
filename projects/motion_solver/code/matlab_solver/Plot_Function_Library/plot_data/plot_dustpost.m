...
function plot_dustpost(dustpost_data, plot_data, leg_list)
   ...
   % Initialize plot max starting and ending time steps
    ...
    for i = 1:length(plot_data.fields)
        ...
        for j = 1:length(dustpost_data) 
            ...
            if (dustpost_data{j}.iter-1)*dustpost_data{j}.dt < t_end
                t{j} = reshape(t_start:dustpost_data{j}.dt:(dustpost_data{j}.iter-1)*dustpost_data{j}.dt,[],1);
            else
                t{j} = reshape(t_start:dustpost_data{j}.dt:t_end,[],1);
            end
            if t_max <= t{j}(end)
                t_max = t{j}(end)+dustpost_data{j}.dt;
            end
        ...
        for j = 1:length(dustpost_data)
            for k = t{j}(1)/dustpost_data{j}.dt+1:t{j}(end)/dustpost_data{j}.dt+1
                for l = 1:length(dustpost_data{j})

                    % Save values from each dust post processing file into the cell
                    val{j}(1,l,k) = dustpost_data{j}.dustpost{l}(k).(plot_data.fields{i}(1) + "x");
                    val{j}(2,l,k) = dustpost_data{j}.dustpost{l}(k).(plot_data.fields{i}(1) + "y");
                    val{j}(3,l,k) = dustpost_data{j}.dustpost{l}(k).(plot_data.fields{i}(1) + "z");
        ...
        for j = 1:length(dustpost_data)
            for k = 1:(length(coord)+plot_data.mag)*(length(nodes{j})+plot_data.sum)
                figure(k);
                plot(t{j}, plot_val{k,j}(t_start/dustpost_data{j}.dt+1:t_start/dustpost_data{j}.dt+length(t{j})));
                title(tit{k});
                xlabel('Time');
