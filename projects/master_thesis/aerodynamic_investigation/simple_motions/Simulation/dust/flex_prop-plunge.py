...
output_file_name   = "flex_prop-plunge"
...
h_range            = [0.05,0.1,0.05]
amp_ratio_range    = [20,40]
...
for i in range(len(h_range)):
    for j in range(len(phase_prop_range)):
        for l in range(len(amp_ratio_range)):
            ...
            if int(h_range[i]) == round(h_range[i],3):
                h_sim = int(h_range[i])
            else:
                h_sim = round(h_range[i],3)
            
            amp_prop_sim = h_sim*amp_ratio_range[l]
            ...
            curr_sim_name = "J"+str(J_sim)+"_h"+str(h_sim)+"_k"+str(k_sim)+"_alphaprop"+str(amp_prop_sim)+"_kprop"+str(kprop_sim)+"_phaseprop"+str(int(phase_prop_range[j]))
...
for i in range(len(new_sim_name)):
    ...
    references_plunge_WP_parameters  = [str(amp[i]*c),str(omega[i]),str(phase),str(offset),str(amp_prop[i]*math.pi/180),str(omega_prop[i]),str(phase_prop[i]),str(offset_prop),str(round(xp*c-c/4,6)),str(round(yp*bby2,6)),str(round(zp*r,6)),str(round(xp_hub*c-c/4,6)),str(2*math.pi*vel/(J*2*r)),rot_axis,parent,ref_tag]
...
for i in range(len(h_range)):
    for j in range(len(phase_prop_range)):
        ...
        curr_sim_name = "J"+str(J_sim)+"_h"+str(h_sim)+"_k"+str(k_sim)+"_alphaprop"+str(amp_prop_sim)+"_kprop"+str(kprop_sim)+"_phaseprop"+str(phase_prop_sim)
        if curr_sim_name.replace(".","_")+"_"+comp_name_1+comp_name_2 not in sim_name:
            new_sim_name += [curr_sim_name.replace(".","_")]
            amp          += [h_sim]
            omega        += [omega_sim]
...
for i in range(len(new_sim_name)):
    ...
    # fileWrapper parameters for current simulation
    references_plunge_WP_parameters  = [str(amp[i]*c),str(omega[i]),str(phase),str(offset),str(amp_prop*math.pi/180),str(omega_prop),str(phase_prop),str(offset_prop),str(round(xp*c-c/4,6)),str(round(yp*bby2,6)),str(round(zp*r,6)),str(round(xp_hub*c-c/4,6)),str(2*math.pi*vel/(J*2*r)),rot_axis,parent,ref_tag]
    dust_pre_WP_parameters           = [os.path.join(wing_file_loc,wing_file),os.path.join(blade_file_dust_pre_loc,"blade.in")]
    dust_parameters                  = [str(tf),str(dt),str(vel),str(density),data_basename]
    dustPost_IntLoads_WP_parameters  = [str(int(round(tf/dt,0))+1),basename_Intloads,data_basename,comp_name_1,comp_name_2]
    dustPost_SecLoads_W_parameters   = [str(int(round(tf/dt,0))+1),basename_Secloads,data_basename,comp_name_1]
...
for i in range(len(new_sim_name)):
    ...
    # fileWrapper parameters for current simulation
    references_plunge_WP_parameters  = [str(amp*c),str(omega[i]),str(phase),str(offset),str(amp_prop*math.pi/180),str(omega_prop[i]),str(phase_prop[i]),str(offset_prop),str(round(xp*c-c/4,6)),str(round(yp*bby2,6)),str(round(zp*r,6)),str(round(xp_hub*c-c/4,6)),str(2*math.pi*vel/(J*2*r)),rot_axis,parent,ref_tag]
    dust_pre_WP_parameters           = [os.path.join(wing_file_loc,wing_file),os.path.join(blade_file_dust_pre_loc,"blade.in")]
    dust_parameters                  = [str(tf),str(dt),str(vel),str(density),data_basename]
    dustPost_IntLoads_WP_parameters  = [str(int(round(tf/dt,0))+1),basename_Intloads,data_basename,comp_name_1,comp_name_2]
    dustPost_SecLoads_W_parameters   = [str(int(round(tf/dt,0))+1),basename_Secloads,data_basename,comp_name_1]
