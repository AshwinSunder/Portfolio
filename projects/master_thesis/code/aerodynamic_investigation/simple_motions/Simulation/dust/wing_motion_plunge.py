...
output_file_name   = "wing_motion-plunge"
...
for i in range(len(h_range)):
    for j in range(len(omega_range)):
        ...
        if int(h_range[i]) == round(h_range[i],2):
            h_sim = int(h_range[i])
        else:
            h_sim = round(h_range[i],2)
        ...
        curr_sim_name = "J"+str(J_sim)+"_h"+str(round(h_sim,4))+"_k"+str(k_sim)+"_alphaprop"+str(int(amp_prop))+"_kprop"+str(kprop_sim)+"_phaseprop"+str(int(phase_prop))
...
for i in range(len(new_sim_name)):
    ...
    # fileWrapper parameters for current simulation
    references_plunge_WP_parameters  = [str(amp[i]*c),str(omega),str(phase),str(offset),str(amp_prop*math.pi/180),str(omega_prop),str(phase_prop),str(offset_prop),str(round(xp*c-c/4,6)),str(round(yp*bby2,6)),str(round(zp*r,6)),str(round(xp_hub*c-c/4,6)),str(2*math.pi*vel/(J*2*r)),rot_axis,parent,ref_tag]
    dust_pre_WP_parameters           = [os.path.join(wing_file_loc,wing_file),os.path.join(blade_file_dust_pre_loc,"blade.in")]
    dust_parameters                  = [str(tf),str(dt),str(vel),str(density),data_basename]
    dustPost_IntLoads_WP_parameters  = [str(int(round(tf/dt,0))+1),basename_Intloads,data_basename,comp_name_1,comp_name_2]
    dustPost_SecLoads_W_parameters   = [str(int(round(tf/dt,0))+1),basename_Secloads,data_basename,comp_name_1]
