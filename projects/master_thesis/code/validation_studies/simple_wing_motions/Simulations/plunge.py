...
omega_plunge_range  = [40*math.pi*x/3 for x in range(1,11)]
amp_plunge_range    = [0.025,0.05,0.1]
...
for j in range(len(comp_names)):
    for i in range(len(amp_plunge_range)):
            for l in range(len(omega_plunge_range)):

                h_sim    = amp_plunge_range[i]

                k_sim         = round(omega_plunge_range[l]*c/(2*vel),3)
                ...
                if curr_sim_name.replace(".","_")+"_"+comp_names[j] not in sim_name:
                    ...
                    amp_plunge   += [round(h_sim*c,4)]
                    omega_plunge += [omega_plunge_range[l]]
...
for i in range(len(new_sim_name)):
    ...
    references_parameters        = [str(amp_pitch),str(omega_pitch),str(phase_pitch),str(offset_pitch),str(amp_plunge[i]),str(omega_plunge[i]),str(phase_plunge),str(offset_plunge)]
